   ...
   */
   ...
   public int selecao() {
     ...
     for (int i = 0; i < (n - 1); i++) {
        ...
        for (int j = (i + 1); j < n; j++){
          com++;
           if (array[menor].getUf().compareTo(array[j].getUf()) == 0 ){
   ...
   public int insercaoPorCor(int cor, int h){
     ...
     Docente d = new Docente();
      for (int i = (h + cor); i < n; i+=h) {
         String tmp = array[i].getUf();
         d =  array[i];
         int j = i - h;
         while ((j >= 0) && (array[j].getUf().compareTo(tmp) > 0)) {
            array[j + h] = array[j];
            j-=h;
            comp++;
         }
...
public class Instituicao{
    ...
    for(j = 0; j<nEntrada; j++){
      ...
      // Somar as comparaçoes de insercao
       comp += lista.shellsort();
