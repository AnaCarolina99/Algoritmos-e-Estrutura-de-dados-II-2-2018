...
class Lista {
   ...
   //Set da variavel comp
   public void setComp(int comp){
     this.comp = comp;
   }//Fim metodo

   //Get da variavel comp
   public int getComp(){
     return this.comp;
   }//Fim metodo
   ...
   public void swap(int i, int j) {
      Docente temp = array[i];
      array[i] = array[j];
      array[j] = temp;
   }//Fim metodo

    //metodo para chamar o metodo de ordenacao radixsort
 ...
 */
     public int radixsortEp(){
         int doido = array[0].getCodigo();
         int exp = 1;

         Docente[] arrayDoido = new Docente[n];

         for (int i = 1; i < n; i++){
           if (array[i].getCodigo() > doido ) {
             doido = array[i].getCodigo();
             comp++;
           }
           comp++;
         }//Fim for
         while (doido / exp > 0){
          ...
          for (int i = 0; i < n ; arrayAux[(array[i].getCodigo() / exp) % 10]++, i++);
          for (int i = 1; i < 10; arrayAux[i] += arrayAux[i - 1], i++);
          for (int i = n - 1; i >= 0; arrayDoido[--arrayAux[(array[i].getCodigo() / exp) % 10]] = array[i], i--);
        ...
        for (int i = 0; i < n; i++){
          for (int j = 0; j < n; j++){
            comp++;
            if( (array[i].getCodigo() == array[j].getCodigo()) ) {
              if ( array[i].getSigla().compareTo(array[j].getSigla()) > 0 ){
                Docente tmp1 = array[i];
                array[i] = array[j];
                array[j] = tmp1;
              }
              if (array[i].getSigla().compareTo(array[j].getSigla()) < 0 ){
                Docente tmp2 = array[j];
                array[j] = array[i];
                array[i] = tmp2;
              }
            }
...
public class Instituicao{
    ...
    //laco para acessar o arquivo e insirir o arquivo no fim da lista
    ...
    //Chamando metodo radizsort
     lista.radixSort();
      ////Imprimir o arquivo ordenado
    lista.mostrar();


    //Criaçao do Log e inicializacao de algumas variaveis
	long fim = System.nanoTime();
    long com = fim - inicio;
	//Criacao e escrita do arquivo log
    Arq.openWrite("591513_radizsort.txt","ISO-8859-1");
	//Printacao das variaveis contidas no arquivo
    Arq.println("591513 \t Tempo:" + com +"s \t Comparaçoes:" +lista.getComp());
