 ...
 */
  public int mediaArr() throws Exception{
        ...
        while((x% array.length) != ultimo){
          soma += array[x].getDespesaDocente();
          x = (x+1) % array.length;
          cont++;
      }
...
public class Instituicao{
    ...
    for(int i = 0; i<nEntrada; i++){
      Docente p = new Docente();
        ...
        fila.inserir(p);
        MyIO.println(fila.mediaArr());
      }// Fim for

//Laco para instanciar Docente e inserir ou remover as linhas desejadas
// e receber a variavel com as medias ja feitas e printa-las
    for(int i = 0; i<num; i++){
        Docente p = new Docente();
          linha = r[i].split(" ");
