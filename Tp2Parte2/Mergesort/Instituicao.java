...
class Lista {
   ...
   */ 
    	public void mergeSort( Docente [] array ) {
		Docente [ ] tmpArray = new Docente[n];
		mergeSort( array, tmpArray, 0, n - 1 );
	}//Fim metodo


  //Metodo recursivo que chama o metodo merge e troca suas posicoes 
   	public void mergeSort( Docente [] array, Docente [] tmpArray, int left, int right ) {
   		if( left < right ) {
   ...
   */
  	...
  	public int merge( Docente [] array, Docente [] tmpArray, int leftPos, int rightPos, int rightEnd ) {
		int leftEnd = rightPos - 1;
		int tmpPos = leftPos;
		int numElements = rightEnd - leftPos + 1;

		while( leftPos <= leftEnd && rightPos <= rightEnd ){
			double ep1 = array[ leftPos ].getReceita(), ep2 = array[ rightPos ].getReceita();
			String str1 = array[ leftPos ].getSigla(), str2 = array[ rightPos ].getSigla();

			if( ep1 < ep2 ){
				comp++;
				tmpArray[ tmpPos++ ] = array[ leftPos++ ];
			} else if(ep1 == ep2 && str1.compareTo(str2) < 0){
				comp++;
				tmpArray[ tmpPos++ ] = array[ leftPos++ ];
