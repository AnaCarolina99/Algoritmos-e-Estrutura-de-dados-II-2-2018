/**
    ...
    *Date:30/08/2018
    *Note: Ajuda de Ana Carolina Meideiros e Ana Luiza no desenvolvimento no codigo
...
class Lista{
   ...
   public boolean pesquisar(Facu x) {
    ...
    public boolean pesquisarSequencia(String s){
      int i = 0;
      boolean resp = false;
      while(i < n && !resp){
        if(array[i].getSigla().equals(s)){
          resp = true;
...
public class PesquisaSequencial {
	...
	public static void main (String[] args) throws Exception {
        ...
        } while (alocacao[nAlocacao++].equals("FIM") == false);
        nAlocacao--;

        for (int i = 0; i < nEntrada; i++) {
            Facu facu = new Facu();
            facu.ler(Integer.parseInt(entrada[i]));
            lista.inserirFim(facu);
        }

        for (int j = 0; j < nAlocacao; j++) {
          String facu = alocacao[j];
          if(lista.pesquisarSequencia(facu)){
            MyIO.println("SIM");
          }else{
            MyIO.println("NÃO");
          }
        ...
        Arq.openWrite("613838_sequencial.txt","ISO-8859-1");
        Arq.println("613838\t" + totalTime + "\t" + lista.comparacao());
        Arq.close();
    }
} 
