...
class Lista {
   ...
   public int selecao() {
     ...
     for (int i = 0; i < k; i++) {
        ...
        for (int j = (i + 1); j < n; j++){
          comp++;
           if (array[menor].getCodigoMunicipio() == (array[j].getCodigoMunicipio())){
   ...
   public  void constroi(int tamHeap){
    ...
    public int reconstroi(int tamHeap){
        int i = 1, filho;
        while(i <= (tamHeap/2)){
      ...
      /**
       * Algoritmo parcial de ordenacao Heapsort.
       * Retorna numero de comparacoes
       */
      public int heapsort() {
        //Alterar o vetor ignorando a posicao zero
        Docente[] tmp = new Docente[n+1];
        for(int i = 0; i < n; i++){
           tmp[i+1] = array[i];
        }
        array = tmp;
   ...
   /**
    * Mostra os elementos feitos da ordenaçao parcial.
    */
   public void mostrar (){
      int k = 10;
      for(int i = 0; i < 10; i++){
